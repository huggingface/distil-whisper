...
2. Clone the `distil_whisper` repository and pip install the required packages from the [requirements.txt](#requirements) file:
...
3. Configure Accelerate by running the following command. Note that you should set the data type (dtype) to your preferred dtype for training (e.g. `bfloat16` on A100 GPUs, `float16` on V100 GPUs, etc.):

```bash
accelerate config
```

4. To confirm that you have a working environment, you can run the following code cell to stream one sample of data from the LibriSpeech dataset, and check that you can perform a forward pass of the "tiny" Whisper model:
...
accelerate run_pseudo_labelling.py \
...
# TODO: configure these arguments
```bash
#!/usr/bin/env bash

accelerate run_distillation.py \
